...
log_file = 'epoch20/output trainig/train convnext.txt'
...
ax1.set_title('Training and Validation Loss - ConvNext', fontsize=14, fontweight='bold')
...
ax2.set_title('Training and Validation Accuracy - ConvNext', fontsize=14, fontweight='bold')
...
if len(epochs) < 20:
    ax1.axvline(x=epochs[-1], color='orange', linestyle='--', alpha=0.7, linewidth=2, label='Early Stopping')
    ax2.axvline(x=epochs[-1], color='orange', linestyle='--', alpha=0.7, linewidth=2, label='Early Stopping')
    ax1.legend(fontsize=11, loc='best')
    ax2.legend(fontsize=11, loc='best')
...
# Save the figure
output_file = 'convnext_training_curves_epoch20.png'
...
ax1_alone.set_title('Training and Validation Loss - ConvNext', fontsize=14, fontweight='bold')
...
if len(epochs) < 20:
    ax1_alone.axvline(x=epochs[-1], color='orange', linestyle='--', alpha=0.7, linewidth=2, label='Early Stopping')
    ax1_alone.legend(fontsize=11, loc='best')
plt.tight_layout()
plt.savefig('convnext_loss_curve_epoch20.png', dpi=300, bbox_inches='tight', facecolor='white')
print(f"[OK] Loss curve saved as: convnext_loss_curve_epoch20.png")
plt.close()
...
ax2_alone.set_title('Training and Validation Accuracy - ConvNext', fontsize=14, fontweight='bold')
...
if len(epochs) < 20:
    ax2_alone.axvline(x=epochs[-1], color='orange', linestyle='--', alpha=0.7, linewidth=2, label='Early Stopping')
    ax2_alone.legend(fontsize=11, loc='best')
plt.tight_layout()
plt.savefig('convnext_accuracy_curve_epoch20.png', dpi=300, bbox_inches='tight', facecolor='white')
print(f"[OK] Accuracy curve saved as: convnext_accuracy_curve_epoch20.png")
plt.close()
