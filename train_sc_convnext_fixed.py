#!/usr/bin/env python3
"""
🚀 FIXED SC-ConvNeXt Training Script - Windows Compatible!
Fixes for execution hanging issues:
- Removed num_workers for Windows compatibility
- Fixed matplotlib backend issues
- Added progress indicators
- Removed interactive plotting
...
def get_data_loaders():
    ...
    # FIXED: Remove num_workers for Windows compatibility
...
def evaluate_model(model, test_loader, device, class_names):
    ...
    try:
        ...
        # FIXED: Don't try to display, just save
        print("💡 Confusion matrix saved to files (no display in non-interactive mode)")
        ...
        for i in range(len(class_names)):
            for j in range(len(class_names)):
                if i != j and conf_matrix[i, j] > max_confusion:
                    ...
                    most_confused = (class_names[i], class_names[j], conf_matrix[i, j])
        
        if most_confused:
            print(f"  {most_confused[0]} → {most_confused[1]}: {most_confused[2]} times")
