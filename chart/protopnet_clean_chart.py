#!/usr/bin/env python3
"""
ProtoPNet Accuracy vs F1-Score Chart (Clean Version)
===================================================
This script creates a clean chart for ProtoPNet
showing accuracy vs F1-score comparison without interpretability block
"""
...
def create_clean_protopnet_chart():
    """Create clean ProtoPNet accuracy vs F1-score comparison chart"""
    
    # Data for ProtoPNet
    epochs = ['10 Epochs', '20 Epochs']
    accuracy = [56.13, 69.98]
    f1_score = [53.55, 70.84]
    
    # Create figure with subplots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))
    fig.suptitle('ProtoPNet: Accuracy vs F1-Score Comparison (10 vs 20 Epochs)', 
    ...
    for i, (acc, f1) in enumerate(zip(accuracy, f1_score)):
        ax2.text(i, acc + 1.0, f'{acc:.2f}%', ha='center', va='bottom', 
    ...
    acc_improvement = ((69.98 - 56.13) / 56.13) * 100
    f1_improvement = ((70.84 - 53.55) / 53.55) * 100
    
    ax2.annotate(f'+{acc_improvement:.1f}%', xy=(0.5, 63), xytext=(0.5, 66),
                arrowprops=dict(arrowstyle='->', color=colors[0], lw=2),
                fontsize=12, fontweight='bold', color=colors[0], ha='center')
    
    ax2.annotate(f'+{f1_improvement:.1f}%', xy=(0.5, 62), xytext=(0.5, 59),
                arrowprops=dict(arrowstyle='->', color=colors[1], lw=2),
                fontsize=12, fontweight='bold', color=colors[1], ha='center')
    
    plt.tight_layout()
    plt.savefig('protopnet_clean_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # Create clean detailed single chart
    create_clean_detailed_chart()

def create_clean_detailed_chart():
    """Create a clean detailed single chart without interpretability block"""
    ...
    # Customize chart
    ax.set_title('ProtoPNet: Accuracy vs F1-Score Performance Comparison', 
    ...
    # Add clean performance statistics (without interpretability)
    acc_improvement = ((69.98 - 56.13) / 56.13) * 100
    f1_improvement = ((70.84 - 53.55) / 53.55) * 100
    
    # Add clean text box with only performance statistics
    stats_text = f"""Performance Improvements (10→20 Epochs):
    
Accuracy: +{acc_improvement:.2f}% ({56.13:.2f}% → {69.98:.2f}%)
F1-Score: +{f1_improvement:.2f}% ({53.55:.2f}% → {70.84:.2f}%)

Training Time: 1.7h → 2.1h (+23.5%)
...
GPU Memory: 2.8GB"""
    ...
    ax.axhline(y=70, color='green', linestyle='--', alpha=0.7, linewidth=2, label='70% Target')
    ax.axhline(y=60, color='orange', linestyle='--', alpha=0.7, linewidth=2, label='60% Baseline')
    
    plt.tight_layout()
    plt.savefig('protopnet_clean_detailed.png', dpi=300, bbox_inches='tight')
    plt.show()

if __name__ == "__main__":
    print("Creating Clean ProtoPNet Accuracy vs F1-Score Comparison Charts...")
    create_clean_protopnet_chart()
    print("Charts generated successfully!")
    print("Files created:")
    print("  - protopnet_clean_comparison.png")
    print("  - protopnet_clean_detailed.png")
