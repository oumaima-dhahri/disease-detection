#!/usr/bin/env python3
"""
ProtoPNet Accuracy vs F1-Score Chart
===================================
This script creates a focused chart for ProtoPNet
showing accuracy vs F1-score comparison at epoch 10 and 20
"""

import matplotlib.pyplot as plt
import numpy as np
import seaborn as sns
import warnings
warnings.filterwarnings('ignore')

# Set style
plt.style.use('seaborn-v0_8')
sns.set_palette("husl")

def create_protopnet_chart():
    """Create ProtoPNet accuracy vs F1-score comparison chart"""
    
    # Data for ProtoPNet
    epochs = ['10 Epochs', '20 Epochs']
    accuracy = [56.13, 69.98]
    f1_score = [53.55, 70.84]
    
    # Create figure with subplots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))
    fig.suptitle('ProtoPNet: Accuracy vs F1-Score Comparison (10 vs 20 Epochs)', 
                fontsize=18, fontweight='bold')
    
    # Colors
    colors = ['#2E86AB', '#A23B72']
    
    # Chart 1: Side-by-side bar comparison
    x = np.arange(len(epochs))
    width = 0.35
    
    bars1 = ax1.bar(x - width/2, accuracy, width, label='Accuracy (%)', 
                   color=colors[0], alpha=0.8, edgecolor='black', linewidth=1.5)
    bars2 = ax1.bar(x + width/2, f1_score, width, label='F1-Score (%)', 
                   color=colors[1], alpha=0.8, edgecolor='black', linewidth=1.5)
    
    ax1.set_title('Performance Metrics Comparison', fontweight='bold', fontsize=14)
    ax1.set_xlabel('Training Configuration', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Performance (%)', fontsize=12, fontweight='bold')
    ax1.set_xticks(x)
    ax1.set_xticklabels(epochs)
    ax1.legend(fontsize=11)
    ax1.grid(True, alpha=0.3, axis='y')
    ax1.set_ylim(50, 75)
    
    # Add value labels on bars
    for bar in bars1:
        height = bar.get_height()
        ax1.text(bar.get_x() + bar.get_width()/2., height + 0.5,
                f'{height:.2f}%', ha='center', va='bottom', 
                fontweight='bold', fontsize=11)
    for bar in bars2:
        height = bar.get_height()
        ax1.text(bar.get_x() + bar.get_width()/2., height + 0.5,
                f'{height:.2f}%', ha='center', va='bottom', 
                fontweight='bold', fontsize=11)
    
    # Chart 2: Line plot showing improvement trend
    ax2.plot(epochs, accuracy, 'o-', color=colors[0], linewidth=3, 
             markersize=12, label='Accuracy (%)', alpha=0.8)
    ax2.plot(epochs, f1_score, 's-', color=colors[1], linewidth=3, 
             markersize=12, label='F1-Score (%)', alpha=0.8)
    
    ax2.set_title('Performance Improvement Trend', fontweight='bold', fontsize=14)
    ax2.set_xlabel('Training Configuration', fontsize=12, fontweight='bold')
    ax2.set_ylabel('Performance (%)', fontsize=12, fontweight='bold')
    ax2.legend(fontsize=11)
    ax2.grid(True, alpha=0.3)
    ax2.set_ylim(50, 75)
    
    # Add value labels on points
    for i, (acc, f1) in enumerate(zip(accuracy, f1_score)):
        ax2.text(i, acc + 1.0, f'{acc:.2f}%', ha='center', va='bottom', 
                fontweight='bold', fontsize=11, color=colors[0])
        ax2.text(i, f1 - 1.5, f'{f1:.2f}%', ha='center', va='top', 
                fontweight='bold', fontsize=11, color=colors[1])
    
    # Add improvement annotations
    acc_improvement = ((69.98 - 56.13) / 56.13) * 100
    f1_improvement = ((70.84 - 53.55) / 53.55) * 100
    
    ax2.annotate(f'+{acc_improvement:.1f}%', xy=(0.5, 63), xytext=(0.5, 66),
                arrowprops=dict(arrowstyle='->', color=colors[0], lw=2),
                fontsize=12, fontweight='bold', color=colors[0], ha='center')
    
    ax2.annotate(f'+{f1_improvement:.1f}%', xy=(0.5, 62), xytext=(0.5, 59),
                arrowprops=dict(arrowstyle='->', color=colors[1], lw=2),
                fontsize=12, fontweight='bold', color=colors[1], ha='center')
    
    plt.tight_layout()
    plt.savefig('protopnet_accuracy_f1_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # Create detailed single chart
    create_detailed_protopnet_chart()

def create_detailed_protopnet_chart():
    """Create a detailed single chart with more information"""
    
    fig, ax = plt.subplots(1, 1, figsize=(12, 8))
    
    # Data
    epochs = ['10 Epochs', '20 Epochs']
    accuracy = [56.13, 69.98]
    f1_score = [53.55, 70.84]
    
    # Colors
    colors = ['#2E86AB', '#A23B72']
    
    # Create grouped bar chart
    x = np.arange(len(epochs))
    width = 0.35
    
    bars1 = ax.bar(x - width/2, accuracy, width, label='Accuracy (%)', 
                   color=colors[0], alpha=0.8, edgecolor='black', linewidth=1.5)
    bars2 = ax.bar(x + width/2, f1_score, width, label='F1-Score (%)', 
                   color=colors[1], alpha=0.8, edgecolor='black', linewidth=1.5)
    
    # Customize chart
    ax.set_title('ProtoPNet: Accuracy vs F1-Score Performance Comparison', 
                fontweight='bold', fontsize=16, pad=20)
    ax.set_xlabel('Training Configuration', fontsize=14, fontweight='bold')
    ax.set_ylabel('Performance (%)', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(epochs, fontsize=12)
    ax.legend(fontsize=12, loc='upper left')
    ax.grid(True, alpha=0.3, axis='y')
    ax.set_ylim(50, 75)
    
    # Add value labels on bars
    for bar in bars1:
        height = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2., height + 0.5,
                f'{height:.2f}%', ha='center', va='bottom', 
                fontweight='bold', fontsize=12)
    for bar in bars2:
        height = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2., height + 0.5,
                f'{height:.2f}%', ha='center', va='bottom', 
                fontweight='bold', fontsize=12)
    
    # Add improvement statistics
    acc_improvement = ((69.98 - 56.13) / 56.13) * 100
    f1_improvement = ((70.84 - 53.55) / 53.55) * 100
    
    # Add text box with statistics
    stats_text = f"""Performance Improvements (10→20 Epochs):
    
Accuracy: +{acc_improvement:.2f}% ({56.13:.2f}% → {69.98:.2f}%)
F1-Score: +{f1_improvement:.2f}% ({53.55:.2f}% → {70.84:.2f}%)

Training Time: 1.7h → 2.1h (+23.5%)
Model Size: 15.2MB (15.2M parameters)
GPU Memory: 2.8GB

Interpretability Focus:
- Prototype-based explanations
- Visual prototype learning
- Explainable AI decisions"""
    
    ax.text(0.02, 0.98, stats_text, transform=ax.transAxes, fontsize=9,
            verticalalignment='top', fontfamily='monospace',
            bbox=dict(boxstyle="round,pad=0.5", facecolor='lightyellow', alpha=0.8))
    
    # Add performance lines
    ax.axhline(y=70, color='green', linestyle='--', alpha=0.7, linewidth=2, label='70% Target')
    ax.axhline(y=60, color='orange', linestyle='--', alpha=0.7, linewidth=2, label='60% Baseline')
    
    plt.tight_layout()
    plt.savefig('protopnet_detailed_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()

if __name__ == "__main__":
    print("Creating ProtoPNet Accuracy vs F1-Score Comparison Charts...")
    create_protopnet_chart()
    print("Charts generated successfully!")
    print("Files created:")
    print("  - protopnet_accuracy_f1_comparison.png")
    print("  - protopnet_detailed_comparison.png")
