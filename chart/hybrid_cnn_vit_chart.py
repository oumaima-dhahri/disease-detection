#!/usr/bin/env python3
"""
Hybrid CNN-ViT Performance Chart
================================
This script creates a focused comparison chart for Hybrid CNN-ViT
showing accuracy and F1-score between 10 vs 20 epochs
"""
...
def create_hybrid_cnn_vit_chart():
    """Create Hybrid CNN-ViT accuracy and F1-score comparison chart"""
    
    # Data
    configurations = ['10 Epochs', '20 Epochs']
    accuracy = [88.45, 89.70]
    f1_score = [88.35, 89.53]
    
    # Create figure with subplots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))
    fig.suptitle('Hybrid CNN-ViT: Accuracy vs F1-Score Comparison (10 vs 20 Epochs)', 
    ...
    acc_improvement = ((89.70 - 88.45) / 88.45) * 100
    f1_improvement = ((89.53 - 88.35) / 88.35) * 100
    
    ax2.annotate(f'+{acc_improvement:.1f}%', xy=(0.5, 88.8), xytext=(0.5, 89.2),
                arrowprops=dict(arrowstyle='->', color=colors[0], lw=2),
                fontsize=12, fontweight='bold', color=colors[0], ha='center')
    
    ax2.annotate(f'+{f1_improvement:.1f}%', xy=(0.5, 88.6), xytext=(0.5, 88.2),
                arrowprops=dict(arrowstyle='->', color=colors[1], lw=2),
                fontsize=12, fontweight='bold', color=colors[1], ha='center')
    
    plt.tight_layout()
    plt.savefig('hybrid_cnn_vit_accuracy_f1_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # Create detailed single chart
    create_detailed_hybrid_chart()

def create_detailed_hybrid_chart():
    ...
    # Customize chart
    ax.set_title('Hybrid CNN-ViT: Accuracy vs F1-Score Performance Comparison', 
...
Accuracy: +{acc_improvement:.2f}% ({88.45:.2f}% → {89.70:.2f}%)
F1-Score: +{f1_improvement:.2f}% ({88.35:.2f}% → {89.53:.2f}%)

Training Time: 1.0h → 2.2h (+120%)
Model Size: 45.8MB (45.8M parameters)
GPU Memory: 5.3GB"""
    ...
    plt.tight_layout()
    plt.savefig('hybrid_cnn_vit_detailed_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()

if __name__ == "__main__":
    print("Creating Hybrid CNN-ViT Accuracy vs F1-Score Comparison Charts...")
    create_hybrid_cnn_vit_chart()
    print("Charts generated successfully!")
    print("Files created:")
    print("  - hybrid_cnn_vit_accuracy_f1_comparison.png")
    print("  - hybrid_cnn_vit_detailed_comparison.png")
