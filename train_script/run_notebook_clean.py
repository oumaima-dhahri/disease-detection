...
def create_clean_script(notebook_path):
    """Create a clean, working script from notebook"""
    
    print(f"Converting {notebook_path} to clean Python script...")
    ...
    clean_script = f"""
# Clean script generated from notebook
import os
import sys
import warnings
...
"""
    ...
    output_file = notebook_path.replace('.ipynb', '_clean.py')
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(clean_script)
    
    print("✅ Clean script created")
    return output_file
...
def run_notebook_silent(notebook_path):
    ...
    if return_code == 0:
        ...
        print("The clean script may have encountered an error.")
...
def run_notebook_debug(notebook_path):
    ...
    # Run the clean script with debug output
    process = subprocess.Popen(
        [sys.executable, output_file],
