...
While classification models identify disease presence, object detection architectures provide precise lesion localization, enabling severity estimation and actionable scouting recommendations. Object detection frameworks such as YOLO, RetinaNet, Mask R-CNN, and EfficientDet enable lesion-level localization, severity assessment, and spatial mapping of disease outbreaks. These approaches allow not only classification but also practical decision-making for treatment and monitoring in agricultural applications (Redmon et al., 2016; Lin et al., 2017; He et al., 2017; Tan et al., 2020).

**YOLO (You Only Look Once) Architecture and Evolution**

The YOLO family of object detectors revolutionized real-time detection through end-to-end single-stage architectures that predict bounding boxes and class probabilities directly from full images in a single forward pass. The original YOLO architecture (Redmon et al., 2016) divided input images into a grid and predicted bounding boxes with confidence scores for each grid cell, achieving real-time inference at 45 frames per second while maintaining competitive accuracy. Subsequent YOLO variants (YOLOv2, YOLOv3, YOLOv4, YOLOv5, YOLOv8, YOLOv9) have progressively improved accuracy and efficiency through architectural innovations including anchor-based detection, feature pyramid networks, CSP (Cross Stage Partial) architectures, and advanced data augmentation strategies (Redmon et al., 2016; Redmon & Farhadi, 2017; Bochkovskiy et al., 2020).

In agricultural disease detection, YOLO variants have been extensively adapted for lesion localization tasks. YOLOv9, the latest iteration, incorporates GELAN (Generalized Efficient Layer Aggregation Network) and PGI (Programmable Gradient Information) mechanisms that enhance multi-scale feature representation and gradient flow, enabling improved detection of lesions across varying sizes—from small early-stage symptoms to large advanced lesions (Qian et al., 2024). YOLO-based systems achieve 80–90% mean Average Precision (mAP) on disease detection benchmarks while maintaining inference speeds of 30–60 frames per second on standard GPUs, making them suitable for real-time field deployment on mobile devices, drones, and edge computing platforms (Qian et al., 2024; Redmon et al., 2016). The primary advantage of YOLO architectures lies in their single-stage design, which eliminates the need for region proposal networks (RPNs) and enables end-to-end training with simplified annotation requirements compared to two-stage detectors.

**RetinaNet: Focal Loss for Dense Object Detection**

RetinaNet, introduced by Lin et al. (2017), addresses the class imbalance problem inherent in dense object detection through the innovative focal loss function, which down-weights easy negative examples and focuses learning on hard examples. The architecture employs a Feature Pyramid Network (FPN) backbone for multi-scale feature extraction and two task-specific subnetworks—one for classification and one for bounding box regression—enabling accurate detection across multiple lesion scales (Lin et al., 2017). RetinaNet achieves state-of-the-art accuracy on standard object detection benchmarks while maintaining competitive inference speeds, making it well-suited for agricultural applications requiring precise lesion localization.

In plant disease detection, RetinaNet's focal loss mechanism is particularly valuable for handling imbalanced datasets where healthy regions vastly outnumber diseased lesions. The architecture's multi-scale FPN backbone enables detection of lesions ranging from small early-stage symptoms (few pixels) to large advanced lesions covering significant leaf area, addressing a critical challenge in agricultural disease detection where lesion size varies dramatically across disease stages and types (Lin et al., 2017; Qian et al., 2024). RetinaNet-based systems typically achieve 82–88% mAP on disease detection tasks, with particularly strong performance on small lesion detection due to the FPN's multi-scale feature representation (Lin et al., 2017). The architecture's two-stage design (feature extraction followed by detection) provides superior accuracy compared to single-stage detectors but at the cost of increased computational complexity and inference latency.

**Mask R-CNN: Instance Segmentation for Precise Lesion Delineation**

Mask R-CNN, introduced by He et al. (2017), extends Faster R-CNN by adding a parallel branch for predicting segmentation masks on each Region of Interest (ROI), enabling pixel-level lesion delineation in addition to bounding box localization. The architecture employs a two-stage detection pipeline: a Region Proposal Network (RPN) generates candidate lesion regions, followed by ROI pooling and parallel branches for classification, bounding box regression, and mask prediction (He et al., 2017). This design enables Mask R-CNN to provide both localization (bounding boxes) and precise segmentation (pixel-level masks) of individual lesion instances, making it particularly valuable for severity estimation through lesion area calculation.

In agricultural disease detection, Mask R-CNN's instance segmentation capability enables precise quantification of disease severity by calculating the percentage of leaf area covered by lesions, a critical metric for treatment decision-making and disease progression monitoring. The architecture's ability to distinguish individual lesion instances within a single image enables accurate lesion counting, which is essential for severity assessment protocols that classify disease severity based on lesion density (He et al., 2017; Qian et al., 2024). Mask R-CNN-based systems achieve 85–92% mAP on disease detection tasks and 78–85% mask Intersection over Union (IoU) on segmentation tasks, providing both accurate localization and precise boundary delineation (He et al., 2017). However, the two-stage architecture's computational overhead (typically 5–10 frames per second on standard GPUs) limits real-time deployment compared to single-stage detectors, making it more suitable for offline analysis or high-performance computing platforms.

**EfficientDet: Scalable and Efficient Object Detection**

EfficientDet, introduced by Tan et al. (2020), combines EfficientNet backbones with a weighted Bi-directional Feature Pyramid Network (BiFPN) and compound scaling to achieve state-of-the-art accuracy with improved efficiency. The architecture's key innovation is the BiFPN, which enables efficient multi-scale feature fusion through weighted bidirectional connections, allowing the network to learn optimal feature fusion strategies across different scales (Tan et al., 2020). EfficientDet employs compound scaling to uniformly scale resolution, depth, and width across backbone, BiFPN, and detection head, enabling a family of models (EfficientDet-D0 through D7) that trade off accuracy and efficiency according to deployment requirements.

In agricultural disease detection, EfficientDet's efficiency-accuracy trade-off makes it particularly valuable for edge deployment scenarios where computational resources are constrained. The EfficientDet-D0 variant achieves 80–85% mAP on disease detection tasks while maintaining inference speeds of 20–30 frames per second on mobile GPUs, making it suitable for real-time field deployment on smartphones and embedded devices (Tan et al., 2020). The architecture's compound scaling strategy enables practitioners to select model variants based on deployment constraints: EfficientDet-D0/D1 for mobile devices, EfficientDet-D2/D3 for edge computing platforms, and EfficientDet-D4/D5 for cloud-based analysis requiring maximum accuracy (Tan et al., 2020). EfficientDet's weighted BiFPN provides superior multi-scale feature representation compared to standard FPNs, enabling improved detection of lesions across varying sizes—a critical requirement in agricultural applications where early-stage symptoms may be only a few pixels in size while advanced lesions may cover significant leaf area.

**Comparative Analysis and Performance Characteristics**

Object detection frameworks exhibit distinct performance characteristics that inform architecture selection for agricultural disease detection applications. Single-stage detectors (YOLO, EfficientDet) prioritize inference speed and real-time deployment, achieving 30–60 frames per second on standard GPUs with 80–90% mAP accuracy, making them ideal for mobile and drone-based field deployment (Redmon et al., 2016; Tan et al., 2020). Two-stage detectors (RetinaNet, Mask R-CNN) prioritize accuracy and precise localization, achieving 82–92% mAP with superior performance on small lesion detection, though at the cost of increased computational overhead (5–15 frames per second) that limits real-time deployment (Lin et al., 2017; He et al., 2017).

For lesion localization tasks requiring bounding boxes only, YOLO variants and RetinaNet provide optimal accuracy-speed trade-offs, with YOLO variants preferred for real-time applications and RetinaNet preferred for offline analysis requiring maximum accuracy on small lesions (Redmon et al., 2016; Lin et al., 2017). For applications requiring pixel-level segmentation and severity estimation, Mask R-CNN provides the necessary instance segmentation capability, enabling precise lesion area calculation and severity quantification (He et al., 2017). For edge deployment scenarios with computational constraints, EfficientDet offers scalable efficiency through compound scaling, enabling practitioners to select model variants that match deployment platform capabilities (Tan et al., 2020).

**Hybrid Detection-Classification Architectures**

Hybrid approaches that combine detection backbones (e.g., YOLO, RetinaNet) with classification networks (e.g., EfficientNet, ResNet) enable both lesion localization and disease classification in a unified framework, achieving 85–90% accuracy on combined detection–classification tasks (Qian et al., 2024). These architectures typically employ a two-stage pipeline: a detection network localizes lesion regions and generates bounding boxes, followed by a classification network that classifies each detected region into specific disease categories. This design enables spatial mapping of disease outbreaks (through detection) while providing detailed disease identification (through classification), supporting comprehensive agronomic decision-making (Qian et al., 2024).

The primary advantage of detection-based approaches is their ability to identify multiple lesions per image, estimate disease severity through lesion count and coverage, and provide spatial context for agronomic decision-making. Detection models enable severity assessment protocols that classify disease severity based on lesion density (number of lesions per unit area) and coverage (percentage of leaf area affected), metrics that are directly actionable for treatment recommendations and monitoring (Qian et al., 2024; Lin et al., 2017; He et al., 2017). However, detection models require more complex annotations (bounding boxes or masks) compared to classification models, increasing annotation costs and training complexity. Annotation requirements vary by architecture: YOLO and RetinaNet require bounding box annotations, Mask R-CNN requires both bounding boxes and pixel-level masks, and EfficientDet requires bounding boxes with optional mask annotations for segmentation variants (Redmon et al., 2016; Lin et al., 2017; He et al., 2017; Tan et al., 2020).

**Practical Considerations for Agricultural Deployment**

For applications where lesion localization and severity estimation are priorities, detection architectures offer significant advantages over pure classification approaches, despite the added annotation and computational overhead (Qian et al., 2024; Lin et al., 2017). Real-time field deployment on mobile devices and drones favors single-stage detectors (YOLO, EfficientDet) that achieve 30–60 frames per second, enabling live disease scouting and immediate treatment recommendations (Redmon et al., 2016; Tan et al., 2020). Offline analysis and research applications requiring maximum accuracy favor two-stage detectors (RetinaNet, Mask R-CNN) that achieve superior performance on small lesion detection and precise segmentation (Lin et al., 2017; He et al., 2017).

Spatial mapping of disease outbreaks through detection enables precision agriculture applications where treatment can be targeted to specific field regions, reducing chemical usage and environmental impact. Severity estimation through lesion count and coverage enables evidence-based treatment decisions aligned with integrated pest management (IPM) protocols that recommend treatment thresholds based on disease severity levels (Qian et al., 2024; He et al., 2017). The combination of detection and classification in hybrid architectures provides comprehensive disease management capabilities, enabling both "where" (localization) and "what" (classification) questions to be answered simultaneously, supporting holistic agronomic decision-making (Qian et al., 2024).
